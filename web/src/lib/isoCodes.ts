/**
 * Mapeamento ISO 3166-1 alpha-2 → numérico (3 dígitos).
 *
 * Usado pra cruzar países identificados pelo DDI do telefone (`countryByDial`
 * retorna alpha-2) com os polígonos do `world-atlas/countries-110m.json`,
 * cujos `id`s são códigos numéricos string ("076" pra Brasil etc.).
 *
 * Lista completa mantida porque o topojson cobre todos os países e queremos
 * lookup O(1) tanto na renderização quanto no tooltip.
 */
export const ISO2_TO_NUM: Record<string, string> = {
  ...
  VG: '092', VI: '850', VN: '704', VU: '548', WF: '876', WS: '882', XK: '983',
  YE: '887', YT: '175', ZA: '710', ZM: '894', ZW: '716',
}

/** Inverso: numérico → ISO-2. Inicializado de forma preguiçosa. */
let _numToIso2: Record<string, string> | null = null

export function numToIso2(numeric: string | number): string | undefined {
  if (!_numToIso2) {
    _numToIso2 = {}
    for (const [iso2, num] of Object.entries(ISO2_TO_NUM)) {
      _numToIso2[num] = iso2
    }
  }
  // Topojson tanto pode trazer "076" quanto 76. Normaliza pra 3 dígitos.
  const key = String(numeric).padStart(3, '0')
  return _numToIso2[key]
}
